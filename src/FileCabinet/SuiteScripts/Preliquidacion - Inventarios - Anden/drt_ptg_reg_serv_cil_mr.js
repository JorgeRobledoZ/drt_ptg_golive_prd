 ...
 define(['SuiteScripts/drt_custom_module/drt_mapid_cm', 'N/runtime', 'N/search', 'N/record', 'N/email', 'N/error', 'N/url', 'N/https'], function (drt_mapid_cm, runtime, search, record, email, error, url, https) {
    ...
    function map(context) {
        try {
            ...
            //log.audit("valoresProceso finally map", valoresProceso);
    ...
    function reduce(context) {
        try {
            ...
            log.debug("idRegistro reduce", idRegistro);

            var objValue = JSON.parse(context.values);
            ...
            log.debug("registroCilindros reduce 1", registroCilindros);
            ...
            var itemCilObj = record.load({
              ...
              log.audit("capacidadArticulo: "+capacidadArticulo, "unidad: "+unidad);
            ...
            for(var z = 0; z < lineasDirecciones; z++){
                ...
                if(idInternoDireccion == idDireccion){
            ...
            log.audit("factorConversion: "+ factorConversion+" precioPorLitro: "+precioPorLitro, 
            "precioPorKilo: "+precioPorKilo, "subTotalSinDecuento: "+subTotalSinDecuento);

            if(tipoArticulo == articuloEnvase){
                subTotalSinDecuento = subTotal;
            }
            ...
            if(descuentoPeso > 0){
                ...
                precioUnitarioFinal = descuentoUnitario * cantidad;
                    ...
                    if((articulo == cilindro10)||(articulo == cilindro20)||(articulo == cilindro30)||(articulo == cilindro45)){
                        ...
                        if(clienteDescuento){
                            for (var i = 1; i < 2; i++) {
                                ...
                                recOportunidad.setCurrentSublistValue("item", "rate", precioUnitarioFinal);
                                recOportunidad.commitLine("item");
                            }
                        }
                    }

                    // log.audit("Hasta aqui quedo 1", "********************");
                    // return;
                    var recOportunidadIdSaved = recOportunidad.save();
            ...
            log.debug("registroCilindros reduce 1 error", registroCilindros);
            log.error({
                title: 'error reduce',
                details: JSON.stringify(error)
            });
            ...
            log.debug("registroCilindros reduce 1 finally", registroCilindros);
        }
    }
